...
static esp_err_t direction_handler(httpd_req_t *req) {
  ...
  if (httpd_req_get_url_query_str(req, request_param, sizeof(request_param)) == ESP_OK) {
    char direction[64];
    if (httpd_query_key_value(request_param, "direction", direction, sizeof(direction)) == ESP_OK) {
  ...
  char response[64];
  snprintf(response, sizeof(response), "Commande : %s", request_param);
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    // Add direction endpoints
    httpd_register_uri_handler(camera_httpd, &direction_uri);
  }
